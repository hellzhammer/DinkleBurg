...
namespace DinkleBurg.Editor_Components.views
{
    public class Vegetation_Menu : View
    {
        public override void Initialize(Vector2 pos, float height, float width)
        {
            ...
            background.Set_Background(Color.Gray,
            Globals.DeviceManager.GraphicsDevice);
            Vector2 last = base_pos;
            var tLabel = new Label("title_label", "Landscape Textures", background.Position, 100, 30,
            Globals.DeviceManager.GraphicsDevice);
            tLabel.Set_Background(Color.Transparent,
            Globals.DeviceManager.GraphicsDevice);
            background.AddChild(tLabel, false, false);

            var h_box = new Box("hbox", new Vector2(background.Position.X, background.Position.Y + tLabel.Height), width, height,
            Globals.DeviceManager.GraphicsDevice);
            ...
            background.AddChild(h_box, centered_items: true);
        }
